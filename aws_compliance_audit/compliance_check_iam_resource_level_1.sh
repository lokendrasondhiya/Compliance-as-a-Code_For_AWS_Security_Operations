...
check_compliance() {
  ...
  compliance_result=$(aws configservice get-compliance-details-by-resource \
    ...
    --output text --profile your-profile-name)
...
iam_users=$(aws iam list-users --query "Users[*].UserName" --output text --profile your-profile-name)
...
iam_roles=$(aws iam list-roles --query "Roles[*].RoleName" --output text --profile your-profile-name)
...
s3_buckets=$(aws s3api list-buckets --query "Buckets[*].Name" --output text --profile your-profile-name)
