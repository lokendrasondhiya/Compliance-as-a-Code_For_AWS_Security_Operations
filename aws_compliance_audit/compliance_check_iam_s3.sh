...
check_compliance() {
  ...
  compliance_result=$(aws configservice get-compliance-details-by-resource --resource-type "$resource_type" --resource-id "$resource_id" --query 'EvaluationResults[*].ComplianceType' --output text)
